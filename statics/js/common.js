;(function () {
                    ...
                    ok: function () {
                        ...
                        $.getJSON(href).done(function (data) {
                            if (data.state === 'success') {
                                ...
                                //art.dialog.alert(data.info); //暂时解决
								alert(data.info);
                            }
                        });
                    },
	...
	*/
	 ...
	 * 后台排序使用的js
     */
    var J_table_sort = $('.J_table_sort');
    if (J_table_sort.length) {
		
		//取得之前的值，因为第一次访问，这两个值是php给到html页面里，而不是在url后面
		var old_sort_field = $(".table-list").data("sort_field");
		var old_sort_by    = $(".table-list").data("sort_by");

		//绑定各链接
		J_table_sort.each(function() {
			$(this).attr("title", "点击按 "+$(this).html()+" 排序");
			$(this).css("cursor", "pointer");
			$(this).css("color", "#2fa4e7");

			//加上箭头
			if (old_sort_field == $(this).data("field") && old_sort_by == "DESC") {
				$(this).append("↓");
			}
			if (old_sort_field == $(this).data("field") && old_sort_by == "ASC") {
				$(this).append("↑");
			}
		});
		...
		if(aQuery.length > 1)
		{
			...
			for(var i=0, iLoop = aBuf.length; i<iLoop; i++)
			{
				...
				//排除sort_field和sort_by字段
				if (aTmp[0] != 'sort_field' && aTmp[0] != 'sort_by' && aTmp[0] != "") {
					//aGET[aTmp[0]] = aTmp[1];
					aLink += aTmp[0]+"="+aTmp[1]+"&";
				}			
			}
		}
		var sort_by = old_sort_by == "DESC" ? "ASC" : "DESC";
		aLink += "sort_by="+sort_by+"&";         

        J_table_sort.on('click', function (e) {
            //e.preventDefault();
			var field = $(this).data("field");
			aLink += "sort_field="+field;
