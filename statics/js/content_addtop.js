...
function change_images2(uploadid, returnid) {
    ...
    $.each(contents, function(i, n) {
        var ids = parseInt(Math.random() * 10000 + 10 * i);
        var filename = filenames[i].substr(0, filenames[i].indexOf('.'));
        str += "<li id='image" + ids + "'><input title='双击查看' type='text' name='" + returnid + "[]' value='" + n + "' style='width:310px;' ondblclick='image_priview(this.value);' class='input'><a href=\"javascript:remove_div('image" + ids + "')\">移除</a> </li>";
    });
...
//前台多图上传，SWF回调函数
function qt_change_images(uploadid, returnid) {
    var d = uploadid.iframe.contentWindow;
    var in_content = d.$("#att-status").html().substring(1);
    var in_filename = d.$("#att-name").html().substring(1);
    var str = $('#' + returnid).html();
    ...
    var images_show = 0;
    $(".qt_change_images").find(".images_show").each(function(){
        if ($(this).data("id") == 1) {
            images_show += 1;
        }
    });
    //限制个数不覆盖
    if (contents.length > images_show) {
        layer.msg("多了"+(contents.length-images_show)+"个");
        return false;
    }
    var status = false;
    $.each(contents, function(i, n) {
        $(".qt_change_images").find(".images_show").each(function(){
            if ($(this).data("id") == 1) {
                if (status == false) {
                    $(this).parents(".del").find(".img_url").val(n);
                    var src = $(this).parents(".del").find(".images_show").attr("src");
                    
                    $(this).parents(".del").find(".images_show").data("src",src);
                    $(this).parents(".del").find(".images_show").attr("src",n);
                    $(this).parents(".del").find(".images_show").data("id",2);
                    status = true;
                }
            }
        });
